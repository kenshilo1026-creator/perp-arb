...
async def run_execution_once() -> None:
    ...
    if not (preview.maker_venue == "variational" and preview.taker_venue == "lighter"):
        raise RuntimeError("run_execution_once currently only supports Variational maker + Lighter taker")
    ...
    quantity = compute_base_quantity_from_clip_usd(
        clip_usd=clip_usd,
        orderbook=taker_book,
    )
    state_machine = ExecutionStateMachine()
    maker_adapter = VariationalBrowserExecutionAdapter()
    taker_adapter = LighterExecutionAdapter(
